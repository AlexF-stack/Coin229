...
export function BottomNav() {
  ...
  return (
    <nav className="safe-pb fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-bg-elevated/95 backdrop-blur-md md:hidden">
        ...
        {links.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              {href === "/panier" && count > 0 && (
                <span className="absolute right-1.5 top-0 flex h-4 min-w-4 items-center justify-center rounded-full bg-coral px-1 text-[10px] font-semibold text-white">
                  {count}
                </span>
              )}
              <span>{label}</span>
